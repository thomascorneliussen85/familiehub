...
function daysSince(iso) {
  return Math.floor((Date.now() - new Date(iso.replace(' ', 'T') + 'Z').getTime()) / (24 * 60 * 60 * 1000));
}
...
export default function FinanceWidget({ onOpen }) {
  ...
  return (
    <section className="panel panel-finance" onClick={onOpen} role="button" tabIndex={0}>
      <div className="panel-header">
        <div className="panel-title">
          <span className="panel-icon">💰</span> Økonomi
        </div>
        <span className="finance-widget-status">{STATUS_ICON[summary.status]}</span>
      </div>
